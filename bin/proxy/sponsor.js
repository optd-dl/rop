...
var router = express.Router(),
    skipper = function skipper(req, res, next) {
    next();
};

router.get('*', function (req, res, next) {
    var param = { user_domain: req.headers.host.replace(new RegExp('(\\w)\\.' + constant.domain + '(:(' + constant.port + '|' + constant.optionPort + ')){0,1}'), "$1", 'i') };
    req.session.locale && (param.lang_flag = i18n.localeMap[req.session.locale]);
    agent.deliver(ropRouter.deliverOption('common', 'sponsor', 'getId', param)).then(function (rawBody) {
        ...
        if (typeof body.is_success == 'boolean' && body.is_success || typeof body.is_success == 'string' && body.is_success == 'true') {
            ...
            res.render('pages/supplier/index', { _session_id: req.session.id, _info: JSON.stringify({ ssv_id: body.ssv_user_id, ssv_name: body.ssv_user_name }), _injection: constant.clientSideScript }, function (error, html) {
...
catRouter.get('*', function (req, res, next) {
    var param = { cat_id: req.url.replace(new RegExp('/api/ApiDetail-(.+).html'), "$1", 'i') };
    req.session.locale && (param.lang_flag = i18n.localeMap[req.session.locale]);
    agent.deliver(ropRouter.deliverOption('common', 'sponsor', 'get_cat_ssv', param)).then(function (rawBody) {
        if (rawBody.error_response) {
            agent.renderErrorHandler(res, rawBody.error_response.msg);
            return;
        }
