...
function packageTree(data) {
    ...
    var xmlObj;
    try{
        xmlObj = $($.parseXML(xml).documentElement);
    }catch(e){
        try{
            var div = document.createElement("div"),tobeRemoved = [];
            div.innerHTML = xml;
            if(div.childNodes && div.childNodes.length){
                for(var i = 0; i < div.childNodes.length; i++){
                    if(div.childNodes[i].nodeName == "#comment"){
                        tobeRemoved.push(div.childNodes[i]);
                    }
                }
                for(var i = 0; i < tobeRemoved.length; i++){
                    div.removeChild(tobeRemoved[i]);
                }
            }
            xmlObj = $($.parseXML(div.innerHTML).documentElement);
        } catch(e1){
            xmlObj = $(xml);
        }
    }
