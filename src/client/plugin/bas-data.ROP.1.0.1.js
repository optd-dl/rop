"object" != typeof JSON && (JSON = {}),
	...
	function(e) {
		...
		/bas\.gif/.test(e.para.server_url) || (e.para.server_url = e.para.server_url.replace(/\/(bas)\\?/, "$1.gif")),
			e.para.debug_mode_url = e.para.debug_mode_url || e.para.server_url.replace("bas.gif", "debug");
		var n = {};
		! function() {
			//获取对象真实类型
			function e(e) {
		...
		g.checkOption = {
			regChecks: {
				regName: /^((?!^cookie_id$|^original_id$|^time$|^properties$|^id$|^first_id$|^second_id$|^users$|^events$|^event$|^user_id$|^date$|^datetime$)[a-zA-Z_$][a-zA-Z\d_$]{0,99})$/i
			},
			...
			test_id: "str",
			group_id: "str",
			cookie_id: function(e) {
				return t.isString(e) && /^.{1,255}$/.test(e) ? !0 : (b.info("cookie_id必须是不能为空，且小于255位的字符串"), !1)
			},
			user_id: function(e) {
				return t.isString(e) && /^.{1,255}$/.test(e) ? !0 : (b.info("user_id必须是不能为空，且小于255位的字符串"), !1)
			}
		}, g.check = function(e) {
			var t = !0;
			for (var r in e)
			...
			t.extend(n, r), t.isObject(r.properties) && !t.isEmptyObject(r.properties) && t.extend(n.properties, r.properties), r.type && "profile" === r.type.slice(0, 7) || t.extend(n.properties, h.getProps(), h.getSessionProps(), t.info.currentProps, t.info.properties()), n.time = time, n.eventTime = eventTime, t.searchObjDate(n), b.info(JSON.stringify(n, null, 4)), e.para.debug_mode === !0 ? this.debugPath(JSON.stringify(n)) : this.serverPath(JSON.stringify(n))
			...
			var n = e.para.server_url;
			var data = JSON.parse(r);
			console.log(data);
			var topic = data.properties.topic ? data.properties.topic : e.para.topic_e;
			console.log(topic);
			var n = -1 !== e.para.server_url.indexOf("?") ? e.para.server_url + "&topic=" + topic : e.para.server_url + "?topic=" + topic;
			
			if (undefined == e.store.getUserId()) {
				data.user_id = e.store.getCookieId();
		...
		var h = e.store = {
				...
				toState: function(e) {
					var t = null;
					return null !== e && "object" == typeof(t = JSON.parse(e)) ? (this._state = t, t.cookie_id) : null
				},
				initSessionState: function() {
					var e = t.cookie.get("basdata2016session"),
						r = null;
				...
				sessionSave: function(e) {
					this._sessionState = e, t.cookie.set("basdata2016session", JSON.stringify(this._sessionState), 0)
				},
				save: function() {
					e.para.cross_subdomain ? t.cookie.set("basdata2016jssdkcross", JSON.stringify(this._state), 730, !0) : t.cookie.set("basdata2016jssdk", JSON.stringify(this._state), 730, !1)
				},
				_sessionState: {},
				_state: {},
				init: function() {
					var r = t.cookie.get("basdata2016jssdk"),
						n = t.cookie.get("basdata2016jssdkcross"),
						i = null;
					e.para.cross_subdomain ? (i = n, null !== r && (b.info("在根域且子域有值，删除子域的cookie"), t.cookie.remove("basdata2016jssdk", !1), t.cookie.remove("basdata2016jssdk", !0)), null === i && null !== r && (b.info("在根域且根域没值，子域有值，根域＝子域的值", r), i = r)) : (b.info("在子域"), i = r), this.initSessionState(), null !== i && this.toState(i) ? e.para.cross_subdomain && null === n && (b.info("在根域且根域没值，保存当前值到cookie中"), this.save()) : (b.info("没有值，set值"), this.set("cookie_id", t.UUID()))
				}
			},
			v = {
				getUtm: function() {
		...
		e.quick = function() {
				...
				"undefined" == typeof e ? h.set("cookie_id", t.UUID()) : g.check({
					cookie_id: e
				}) ? r === !0 ? h.set("cookie_id", e) : h.change("cookie_id", e) : b.info("identify的参数必须是字符串")
			},
			e.userIdentify = function(e, r) {
				"undefined" == typeof e ? h.set("user_id", t.UUID()) : g.check({
					user_id: e
				}) ? r === !0 ? h.set("user_id", e) : h.change("user_id", e) : b.info("userIdentify的参数必须是字符串")
