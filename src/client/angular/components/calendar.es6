...
(function (angular) {
    ...
    TimeSpinnerCtrl.prototype.attachTimeSpinnerEventListeners = function () {
        this.$element.on('keydown', angular.bind(this, this.handleKeyEvent));
        var self = this;
        this.$hourIncreaserElement.on('mousedown',function(event) {
        ...
        this.$minuteIncreaserElement.on('mousedown',function(event) {
            self.holdingSpinner = true;
            self.repeater && self.$interval.cancel(self.repeater);
            self.repeater  = self.$interval(function(){self.incrementMinute(1)}, 200);
        });
        this.$secondIncreaserElement.on('mousedown',function(event) {
            self.holdingSpinner = true;
            self.repeater && self.$interval.cancel(self.repeater);
            self.repeater  = self.$interval(function(){self.incrementSecond(1)}, 200);
        });
        this.$hourDecreaserElement.on('mousedown',function(event) {
            self.holdingSpinner = true;
            self.repeater && self.$interval.cancel(self.repeater);
            self.repeater  = self.$interval(function(){self.incrementHour(-1)}, 200);
        });
        this.$minuteDecreaserElement.on('mousedown',function(event) {
            self.holdingSpinner = true;
            self.repeater && self.$interval.cancel(self.repeater);
            self.repeater  = self.$interval(function(){self.incrementMinute(-1)}, 200);
        });
        this.$secondDecreaserElement.on('mousedown',function(event) {
            self.holdingSpinner = true;
            self.repeater && self.$interval.cancel(self.repeater);
            self.repeater  = self.$interval(function(){self.incrementSecond(-1)}, 200);
        });
