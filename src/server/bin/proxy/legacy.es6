 ...
 */
let express = require('express'),agent = require('../agent'), ropRouter = require('../router'), log4js = require('log4js'),constant = require("../constant"),i18n = require('../i18n');;
let router = express.Router(),info = {ssv_id:"b4ffce1c-0b1a-4e52-b244-d7e13ec6aeac", ssv_name:"", cat_id:""};

router.get('/ApiSdk', (request, response) => {
    response.redirect(`${constant.protocol}://${constant.domain}:${constant.port}/welcome/sdkTool`);
});

router.get('/doc_detail.html', (request, response) => {
    let id = (request.query && request.query.id)?request.query.id:"";
    response.redirect(`${constant.protocol}://${constant.domain}:${constant.port}/welcome/doc/${id}?preview=true`);
});


router.use('/:func-:id.html', (request, response, next) =>{
    let id = request.params.id, func = request.params.func, param={};
...
router.get('/ApiMethod-:id.html', (request, response) => {
    let id = request.params.id, param = {_session_id: request.session.id,_partial:"API",_param:JSON.stringify({apiMethod:id}),_info:JSON.stringify(info),_injection: constant.clientSideScript};
    response.render('pages/supplier/index',param, (error, html) => {
        agent.renderErrorHandler(response, error, html)
    });
});
router.get('/ApiPreview-:id.html', (request, response) => {
    let id = request.params.id, param = {_param:JSON.stringify({apiPreview:id}),_info:JSON.stringify(info),_injection: constant.clientSideScript};
    response.render('pages/supplier/preview',param,(error, html) => {
        agent.renderErrorHandler(response, error, html)
    });
});
router.get('/ApiDomain-:id.html', (request, response) => {
    let id = request.params.id, param = {_session_id: request.session.id,_partial:"API",_param:JSON.stringify({domainMethod:id}),_info:JSON.stringify(info),_injection: constant.clientSideScript};
    response.render('pages/supplier/index',param,(error, html) => {
        agent.renderErrorHandler(response, error, html)
    });
});
router.get('/ApiDomainPreview-:id.html', (request, response) => {
    let id = request.params.id, param = {_param:JSON.stringify({domainPreview:id}),_info:JSON.stringify(info),_injection: constant.clientSideScript};
    response.render('pages/supplier/preview',param,(error, html) => {
        agent.renderErrorHandler(response, error, html)
    });
});
