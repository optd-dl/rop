let Canvas = require('canvas'),agent = require('./agent'),constant = require('./constant');
...
module.exports = params => {
    ...
    return (req, res, next) => {
        if((req.url != params.url) && !(new RegExp(`${params.url}([?](\\w+=\\w+)?(&\\w+=\\w+)*){0,1}$`).test(req.url)))
            return next();
        let canvas = new Canvas(params.canvasWidth, params.canvasHeight);
