...
let secret = "7AE0800C-BBF9-44CD-A568-460E75C16DD4", viewDir = "pages", templateDir = "template";
...
app.get('(/\\w*)*.(png|jpeg|psd|ico|gif|jpg)', (request, response) => {
...
app.get('(/\\w*)*.((?!html)\\w)*', (request, response) => {
...
app.get('/frame/:path([\\/\\w]*)', (request, response) => {
    ...
    if(path == "ApiTool/index"){
        ...
        let timestamp = Math.ceil(Date.parse(new Date()) / 1000), rawSign = `${secret + (request.session.profile ? 'session_id' + request.session.profile.login_user_id + 'sub_session_id' + request.session.profile.login_sub_user_id : "")}timestamp${timestamp}${secret}`;
...
app.get('(/\\w*)*.(\\w)+', (request, response) => {
    agent.logger.warn(`找不到文本${request.url}, 将返回404提示文本`);
    response.status(404).send("没有找到页面");
});

app.get('/_template/:path', (request, response) => {
    let path = request.params ? request.params.path : "";
...
app.get('/sse/:module/:api', (request, response) => {
    let _module = request.params ? request.params.module : "common",
    ...
    let runner = (once)=>{
        agent.deliver(router.deliverOption("sse",_module,_api, param)).then(rawBody => {
...
app.get('/:module', (request, response) => {
...
app.get('/:module/*', (request, response, next) => {
    let module = request.params.module ? request.params.module : 'welcome';
    if(module == '_view'){
        next();
        return;
...
app.get('/_view/:module/:partial', (request, response) => {
    let module = request.params.module ? request.params.module : 'welcome', partial = request.params.partial ? request.params.partial : 'index';
    ...
    if (!initOptions || !Object.keys(initOptions).length) {
        response.render(`${viewDir}/${module}/partials/${partial}`, {}, (error, html) => {
            agent.renderErrorHandler(response, error, html);
        });
        return;
    }
    ...
    request.session.locale && (param.lang_flag = i18n.localeMap[request.session.locale]);
    agent.massDeliver(router.initDeliverOptions(module, partial, param)).then(rawTmplParam => {
        let tmplParam = {};
        ...
        response.render(`${viewDir}/${module}/partials/${partial}`, tmplParam, (error, html) => {
            agent.renderErrorHandler(response, error, html);
        });
    }).done(() => {
    }, why => {
...
app.post('/agent', upload.array(), (request, response) => {
    ...
    if (request.body) {
        ...
        try{
            agent.deliver(router.deliverOption(request.body.module, request.body.partial, request.body.api, param)).then(rawBody => {
